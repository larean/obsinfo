...
class Datalogger(InstrumentComponent):
    ...
    def __init__(self, equipment, sample_rate, response_stages=[],
        ...
        self.delay_correction = delay_correction_value if apply_delay_correction else None
    ...
    def dynamic_class_constructor(cls, info_dict, selected_config, selected_sn):
        ...
        apply_delay_correction = reconfigure_component('apply_delay_correction', info_dict, selected_config, selected_sn, False)
                
        if not apply_delay_correction:
            delay_correction_value = None #This will not change the delays in filters
        else:
            delay_correction_value = reconfigure_component('delay_correction_value', info_dict, selected_config, selected_sn, None)
        
        response_stages = ResponseStages(response_stages, apply_delay_correction)
        #Apply total correction if apply_delay_correction
        response_stages[len(response_stages) - 1].filter.delay = delay_correction_value
